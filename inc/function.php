...
function viewall_loans(){
    ...
    if(isset($_POST['loan_search'])){
        ...
        $fetch_loan = $con->prepare("SELECT * FROM loan_requests WHERE u_email=? ORDER BY 1 DESC");
        $fetch_loan->setFetchMode(PDO:: FETCH_ASSOC);
        $fetch_loan->execute([$email]);
   ...
   while($row = $fetch_loan->fetch()):
        ...
        echo "<tr>
                ...
                <td>".$row['l_status']."</td>";
                if($status=='PENDING'){
                    echo"<td><a href='accounts.php?cancel_id=".$row['loan_id']."'>CANCEL</a></td>";
...
function viewall_disapproved(){
    ...
    if(isset($_POST['loan_search'])){
        ...
        $fetch_loan = $con->prepare("SELECT * FROM loan_requests where l_status=? AND u_email=? ORDER BY 1 DESC");
        $fetch_loan->setFetchMode(PDO:: FETCH_ASSOC);
        $fetch_loan->execute(["DISAPPROVED",$email]);
  ...
  function viewall_cancelled(){
    ...
    if(isset($_POST['loan_search'])){
        ...
        $fetch_loan = $con->prepare("SELECT * FROM loan_requests where l_status=? AND u_email=? ORDER BY 1 DESC");
        $fetch_loan->setFetchMode(PDO:: FETCH_ASSOC);
        $fetch_loan->execute(["CANCELLED",$email]);
  ...
  function viewall_approved(){
    ...
    if(isset($_POST['loan_search'])){
        ...
        $fetch_loan = $con->prepare("SELECT * FROM loan_requests where l_status=? AND u_email=? ORDER BY 1 DESC");
        $fetch_loan->setFetchMode(PDO:: FETCH_ASSOC);
        $fetch_loan->execute(["APPROVED", $email]);
  ...
  function viewall_withdrawn(){
    ...
    if(isset($_POST['loan_search'])){
        ...
        $fetch_loan = $con->prepare("SELECT * FROM loan_requests where l_status=? AND u_email=? ORDER BY 1 DESC");
        $fetch_loan->setFetchMode(PDO:: FETCH_ASSOC);
        $fetch_loan->execute(["WITHDRAWN", $email]);
  ...
  function viewall_cleared(){
    ...
    if(isset($_POST['loan_search'])){
        ...
        $fetch_loan = $con->prepare("SELECT * FROM loan_requests where l_status=? AND u_email=? ORDER BY 1 DESC");
        $fetch_loan->setFetchMode(PDO:: FETCH_ASSOC);
        $fetch_loan->execute(["CLEARED",$email]);
...
function view_loan(){
    ...
    $fetch_loan=$con->prepare("SELECT * FROM loan_requests WHERE loan_id=? AND u_email=?");
    $fetch_loan->setFetchMode(PDO:: FETCH_ASSOC);
    $fetch_loan->execute([$loan_id,$email]); 
...
function cancel_loan(){
    ...
    if(isset($_GET['cancel_id'])){
        ...
        $u_id=$row_user['u_id'];
            ...
            if ($delete_lin->execute(["CANCELLED", $loan_id, $u_id])){
                echo"<script>alert('Loan had been cancelled');</script>";
                echo"<script>window.open('cancelled.php','_SELF');</script>";
            }
        }
}
?>